...
import { formatKitchenContextForAI, formatDietaryForAI, formatUnitPreferenceForAI, type KitchenContext } from './kitchen-context';
...
export async function adjustRecipeForSkillLevel(
  ...
  const dietaryPrompt = formatDietaryForAI(dietaryPreferences);
  const dietaryBlock = dietaryPrompt ? `\n\n${dietaryPrompt}` : '';
  const unitPrompt = formatUnitPreferenceForAI(unitPreference);
  const unitBlock = unitPrompt ? `\n\n${unitPrompt}` : '';
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `${levelPrompt}${kitchenBlock}${dietaryBlock}${unitBlock}
